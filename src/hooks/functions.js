...
//         mainMenu: function() {
//             if (l.length > 0) {
//                 l.find(".dropdown, .dropdown-submenu").prepend('<span className="dropdown-arrow"></span>');
//                 var a = t('#mainMenu nav > ul > li.dropdown > a[href="javascript:void(0)"], #mainMenu nav > ul > li.dropdown > .dropdown-arrow, .dropdown-submenu > a[href="javascript:void(0)"], .dropdown-submenu > .dropdown-arrow, .dropdown-submenu > span, .page-menu nav > ul > li.dropdown > a'),
...
//                             i.attr("data-bg-image") && (e = i.attr("data-bg-image")), i.prepend('<div className="kenburns-bg" style="background-image:url(' + e + ')"></div>')
//                         }
...
//                             rightToLeft: INSPIRO.core.rtlStatus()
//                         }, a.options.itemsLg = a.options.itemsLg || Math.min(Number(a.options.items), Number(4)), a.options.itemsMd = a.options.itemsMd || Math.min(Number(a.options.itemsLg), Number(3)), a.options.itemsSm = a.options.itemsSm || Math.min(Number(a.options.itemsMd), Number(2)), a.options.itemsXs = a.options.itemsXs || Math.min(Number(a.options.itemsSm), Number(1)), s(), a.find("> *").wrap('<div className="polo-carousel-item">'), a.hasClass("custom-height")) {
...
//                         d = t(this).serialize();
//                     !1 === i[0].checkValidity() ? (n.stopPropagation(), i.addClass("was-validated")) : (t(i).removeClass("was-validated"), s.html('<i className="icon-loader fa-spin"> </i> Sending...'), t.ajax({
//                         url: r,
//                         type: l,
//                         data: d,
//                         success: function(n) {
...
//                     i.prepend('<div className="parallax-container" data-bg="' + e + '"  data-velocity="' + a + '" style="background: url(' + e + ')"></div>');
//                     new LazyLoad({
...
//             i && i.each(function() {
//                 t(this).wrap('<div className="embed-responsive embed-responsive-16by9"></div>')
//             })
//         },
//         counters: function() {
//             var i = t(".counter");
...
//                         a.html(t.strftime('<div className="countdown-container"><div className="countdown-box"><div className="number">%-D</div><span>Days</span></div><div className="countdown-box"><div className="number">%H</div><span>Hours</span></div><div className="countdown-box"><div className="number">%M</div><span>Minutes</span></div><div className="countdown-box"><div className="number">%S</div><span>Seconds</span></div></div>'))
//                     })
//                 })
//             }
//         },
...
//                     s.animate({
//                         width: o + "%"
//                     }, "easeInOutCirc").addClass("progress-animated"), s.delay(n).append('<span className="progress-type">' + r + '</span><span className="progress-number animated fadeIn">' + o + "</span>")
//                 };
...
//                     var i = t(this);
//                     i.options = {
//                         barColor: i.attr("data-color") || "#02b0f3",
...
//             var i = t(".map");
//             if (i.length > 0) {
//                 if (void 0 === t.fn.gmap3) return INSPIRO.elements.notification("Warning", "jQuery gmap3 plugin is missing, please go to this <Link to='//support.inspirothemes.com/help-center/articles/8/14/65/google-maps'>help article</Link>and follow instructions on how to configure google maps.", "danger"), !0;
//                 i.each(function() {
//                     var i, e = t(this);
//                     e.options = {
...
//                     }).overlay({
//                         position: [Number(e.options.latitude), Number(e.options.longitude)],
//                         content: '<div className="animated-dot"></div>'
...
//                     }
//                     a.append('<div className="grid-loader"></div>');
...
//                 M = t('<div className="infinite-scroll-message"><p className="animated visible fadeIn">No more posts to show</p></div>');
...
//                     type: a || "warning",
//                     delay: o || 1e4,
//                     template: '<div data-notify="container" ' + p + ' className="bootstrap-notify ' + u + ' alert alert-{0}" role="alert"><button type="button" aria-hidden="true" className="close" data-notify="dismiss">×</button><span data-notify="icon"></span> <span data-notify="title">{1}</span> <span data-notify="message">{2}</span></div>',
//                     mouse_over: !0,
//                     allow_dismiss: !0,
//                     placement: {
//                         from: n
...
//                 }, i.options.controls ? i.options.controls = ' controls="' + i.options.controls + '" ' : i.options.controls = "", i.prepend('<div className="html5vid" id="video-' + i.options.randomId + '"><video playsinline ' + i.options.controls + ' loop="' + i.options.loop + '" muted="' + i.options.muted + '" poster="' + i.options.poster + '" preload="' + i.options.preload + '"><source src="' + i.options.src + '" type="video/mp4"></video></div>'), i.options.autoplay && setTimeout(function() {
...
//                         itemTemplate: '<Link to="{{image}}" title="{{title}}"><img src="{{image_s}}" alt="{{title}}" /></Link>'
//                     }, i.jflickrfeed({
...
//                         n = "grid-" + a;
//                     i.append('<div id="instagram-cnt" className="' + n + '"></div>'), jQuery.fn.spectragram.accessData = {
//                         accessToken: s
...
//                         l = t(this).serialize();
//                     !1 === i[0].checkValidity() ? (o.stopPropagation(), i.addClass("was-validated")) : (t(i).removeClass("was-validated"), a.html('<i className="icon-loader fa-spin"></i>'), t.ajax({
//                         url: n,
//                         type: r,
//                         data: l,
//                         dataType: "json",
